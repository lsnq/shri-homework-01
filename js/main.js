...
(function() {
    ...
    const el = document.getElementById('card-template');
    ...
    data.forEach((item) => {
        ...
        if (item.image) {
            ...
            img.srcset = item.image.replace('.png', '@2x.png') + ' 1x, ' + item.image.replace('.png', '@3x.png') + ' 2x';
            card.querySelector('.image__holder').appendChild(img);
            if (!item.description) {
                const tools = card.querySelector('.tools');
                card.querySelector('.image__holder').appendChild(tools);
            }
        } else {
            card.querySelector('.image').remove();
            card.querySelector('.card').classList.add('card__no-image')
        }
